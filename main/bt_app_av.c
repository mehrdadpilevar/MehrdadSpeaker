...
void apply_lpf(float32_t *input, float32_t *output, uint16_t len) {
    static arm_fir_instance_f32 lpf;
    static float32_t state[FILTER_ORDER + BLOCK_SIZE - 1];
    
    // مقداردهی اولیه فیلتر
    arm_fir_init_f32(&lpf, FILTER_ORDER, lpf_coeffs, state, len);
    arm_fir_f32(&lpf, input, output, len);
}
